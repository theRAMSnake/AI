...
std::string to_string(const PreviewSnapshot& src)
{
    boost::property_tree::ptree tree;
    tree.put("bestFitness", src.bestFitness);
...
std::string to_string(const std::vector<PopInfo>& src)
{
    boost::property_tree::ptree tree;

    for(auto& p : src)
    {
        tree.put(std::to_string(p.id) + ".complexity", p.complexity);
        tree.put(std::to_string(p.id) + ".fitness", p.fitness);
    }
...
std::string executeOperation(const boost::property_tree::ptree& tr)
{
    ...
    else if(opName == "createProject")
    {
        ...
        auto pg = getOrThrow<std::string>(tr, "playground");
        auto engine = getOrThrow<std::string>(tr, "engine");

        gHost.createProject(name, engine, pg);
    ...
    {
        auto fitness = getOrThrow<int>(tr, "fitness");
        auto time = getOrThrow<int>(tr, "minutes");

        gHost.run({static_cast<size_t>(time), fitness});
    ...
    else if(opName == "getLastSnapshot")
    {
        return to_string(gHost.getLastSnapshot());
    }
    else if(opName == "describePopulation")
    {
        return to_string(gHost.describePopulation());
    }
    else if(opName == "exportIndividual")
    {
        auto id = getOrThrow<PopId>(tr, "id");
        return to_string(gHost.exportIndividual(id));
    }
    else if(opName == "perform")
    {
        auto id = getOrThrow<PopId>(tr, "id");
        return gHost.perform(id);
    }
    else if(opName == "set")
    {
