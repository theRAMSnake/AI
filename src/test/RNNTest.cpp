...
{  
   ...
   BOOST_CHECK_EQUAL(0.92414181997875655, gacommon::activate(*n, {0, 0})[0]);
   BOOST_CHECK_EQUAL(1.8482836399575131, gacommon::activate(*n, {0, 0})[0]);
   BOOST_CHECK_EQUAL(2.7724254599362697, gacommon::activate(*n, {0, 0})[0]);
}

BOOST_FIXTURE_TEST_CASE( TestRNN1, RNNTest )
{
   gacommon::NeuroNet2::Node n0 = {
      0, 0.0, -1,
      {
         { 3, 0.12 }
      },
   ...
   std::vector<gacommon::NeuroNet2::Node> nodes = { n0, n1, n2, n3, n4 };
   std::size_t numConnections = 7;


   gacommon::NeuroNet2::Matrix matrix(nodes, numConnections);
