...
iT SparseMatrix<iT,fT>::findIndex(iT row, iT col) const {
	typename vector<iT>::const_iterator jit = std::find(ja_.begin() + ia_.at(row),
...
std::set<std::pair<iT,iT> > SparseMatrix<iT,fT>::sparsityPattern(void) const {
	...
	for (iT i=0; i<Dim(); i++)
		for (iT j=ia_.at(i); j<ia_.at(i+1); j++)
			...
			if (std::abs(a_.at(j))>eps_) ret.insert(make_pair(i,ja_.at(j)));
...
template class SparseMatrix<unsigned int,double>;
